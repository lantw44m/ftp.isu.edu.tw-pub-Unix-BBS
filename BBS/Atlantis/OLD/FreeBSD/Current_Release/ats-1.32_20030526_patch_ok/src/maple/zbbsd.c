...
/* update : 02/09/09 (Dopin for multiport stations)      */
/*                   in.zbbsd -> zbbsd                   */
/*-------------------------------------------------------*/
/* syntax : (1) login as root                            */
/*          (2) zbbsd {ports} [bbsuser] [bbs-public-user-without-passwd]
*/
/*-------------------------------------------------------*/
...
#define YEA 1                                /* Dopin: 這些部份要核對一下 */
#define PASSWD "請按Enter繼續..."
#define BBSUSER "bbsrs"
#define BBSUSER2 "bbsrs"
#define BAD_HOST BBSHOME"/etc/bad_host"      /* 到這邊 */
...
#undef DEBUG_MODE      /* Dopin: 站要維護時 define 後 make zbbsd update */
#define HASHING         /* 然後 kill -HUP 或是直接重開機 連進工作站維護  */
#undef HAVE_CHKLOAD
#define NUMTTYS         256     /* 這裡要注意實際數值 */
#define QLEN            5

#ifdef DEBUG_MODE
#define MSG_WORKING     "\r亞站目前升級資料結構中,請暫時不要登錄\n"
#else
#define MSG_WORKING     "\r進入亞特蘭提斯的世界...\n"
#endif
...
#ifdef  HAVE_CHKLOAD
#define BANNER  "\r\n【亞特蘭提斯】◎ ACG ◎ (ats.twbbs.org)\r\nFreeBSD 5.0 Current(%s) [140.129.66.189:%s]\r\n\n"
#else
#define BANNER  "\r\n\n"
#endif
...
static void ipc_err(char *name) {
  fprintf(stderr, "[%s] error\n", name);
  exit(1);
}

static void resolve_pptshm(void) {
  register int i;
...
void logout(register char *line) {
  int fd;
...
void getremotename(struct sockaddr_in *from, char *rhost, char *rname) {
  ...
  s = sizeof our_sin;
  if(getsockname(0, (struct sockaddr*)&our_sin, &s) < 0) return;
  ...
  *busy = 1;
  ...
  if(ppt_shm->uptime <= now) {
    for(i = visit = 0, pid = ppt_shm->ppt; i < NUMTTYS; i++, pid++) {
      if(p = *pid) {
        errno = 0;
        if((kill(p, 0) < 0) && (errno == ESRCH)) *pid = 0;
        else visit++;
      }
    }
    ppt_shm->visit = visit;
  ...
  if(visit < 500) {
    ...
    do {
      if(!*pid) {
#ifdef BSD44
        line[sizeof("/dev/ptyp") - 2] = "pqrsPQRS"[(i / 32) % 8];
...
void start_client(void) {
  ...
  if(bad_host(rhost) && !strcmp(rname, "?")) exit(1);
  ...
  if(p <= 0) fatal(net, "All network ports in use");
#endif                          /* HASHING */
...
#else                           /* TIOCSCTTY */
  i = t;
  if((t = open(line, O_RDWR)) < 0) _exit(1);
  close(i);
#endif                          /* TIOCSCTTY */
...

{
   ...
   if(pwd = getpwnam(bbsuser)) salt = pwd->pw_passwd;
   else salt = "xx";
  ...
  if(setgid(bbsgid)) exit(1);
  initgroups(bbsuser, bbsgid);

  if(setuid(bbsuid)) exit(1);
  ...
  environ[0] = 0;
  if(terminaltype) putenv(terminaltype);
  ...
  sprintf(hisopts, "%u", from_port);
  execl(bbsprog, "bbs", rhost, line, rname, hisopts, NULL);
...
#define loaddouble(la) ((double)(la) / (1 << 8))
  ...
  for(i = 0; i < 3; i++) cpu_load[i] = loaddouble(avenrun[i]);
#endif

  i = cpu_load[0];
  if(i < limit) i = 0;
  sprintf(maple, "系統負載 %.2f %.2f %.2f%s", cpu_load[0], cpu_load[1],
         cpu_load[2], (i ? "，請稍後再來\n" : ""));
...
#endif
  if((fp = open(buf, O_WRONLY | O_CREAT | O_TRUNC, 0664)) >= 0) {
    sprintf(buf, "%5d\n", getpid());
    write(fp, buf, 6);
    close(fp);
  }
...
int main(int argc, char *argv[]) {
  ...
  if(argc > 1) {
    ...
    for(nfds = 1; nfds < argc; nfds++) {
      ...
      if(csock > 0) msock = bind_port(csock);
      else break;
    }
    ...
    if(nfds < argc) bbsuser = argv[nfds++];
    if(nfds < argc) bbsuser2 = argv[nfds++];
  }
  else {
...
forever:
    ...
    if(msock < 0) {
      if(errno != EINTR) sleep(5);
      continue;
    }
    else if(msock == 0) continue;        /* No network traffic */
