...
/* Dopin: 讓 BBS 支援 RFC 2045 輸入 (by PaulLiu.bbs@processor.tfcis.org) */
char mm_charset[128], mm_encode;
int mm_len;

char mm_getcontentencode(const char *a) {
  int i;
  if(a==NULL) return '\0';
  for(i=0 ; a[i]!='\0' && isspace(a[i]) ; i++);
  return a[i];
}

void mm_getcontentcharset(const char *a, char *charset, int charset_size) {
  const char *b;
  char delim='\0';
  int i=0;
  ...
  *charset = '\0';
  ...
  for( ; *b != '\0' && *b != delim && !isspace(*b) ; b++) {
     if(*b == '\"') {
        delim = *b;
        continue;
     }

     if(i+1>=charset_size) break;
     charset[i] = *b;
     charset[i+1]='\0';
     i++;
  ...
  if(!strcasecmp(charset, "iso-8859-1")) *charset='\0';
}
/* Dopin */
...
bbspost_write_post(int fh, char *board, char *filename) {
  ...
  if(ptr = strchr(buf, '@')) {
     ...
     ptr++;
     for(ci = 0 ; (unsigned char)ptr[ci] > ' ' && ci < 128 ; ci++) ;
     ptr[ci] = 0;
  }
  /* Dopin: 比方 <gghh@ms59.hinet.net> -> ms59.hinet.net QQ;;; */
  /* 稍微改一下 可以改成超兇悍擋法 用部份比對把 DN/IP 擋掉 (如 .com :p) */
  /* 不過不在本篇討論範圍 XD */

  if(ptr)
     if(belong(BBSHOME "/innd/bad_hosts", ptr)) {
        append_flag = 0;
        goto SKIP_CONTENT;
     }
...
#ifndef MapleBBS
  if(POSTHOST != NULL) fprintf(fhfd, "Origin: %.70s\n", POSTHOST);
#endif

  /* Dopin: 讓 BBS 支援 RFC 2045 輸入 (by PaulLiu.bbs@processor.tfcis.org) */
  mm_encode = mm_getcontentencode(HEADER[MIME_CONTENTENCODE_H]);
  mm_getcontentcharset(HEADER[MIME_CONTENTTYPE_H],
                      mm_charset,sizeof(mm_charset)-1);
  mm_len = strlen(BODY);
  if(toupper(mm_encode) =='Q' || toupper(mm_encode) == 'B') {
    int tmpi;
    tmpi = mmdecode(BODY, mm_encode, BODY, 1);
    BODY[tmpi] = '\0';
  }

  if(mm_charset[0] != '\0') {
    char *NEWBODY = strdup(BODY);
    str_iconv_m3(mm_charset, "big5", NEWBODY, strlen(NEWBODY), BODY, mm_len);
    free(NEWBODY);
  }
 /* Dopin */
  ...
  char *pathname, *firstpath;
{
  ...
  if (userid[IDLEN])
    strcpy(&userid[IDLEN], ".");
  strcpy(header.owner, append_flag ? userid : "<AntiSpam>");
  strncpy(header.title, append_flag ? subject : "Spam Mail Address", TTLEN);
  header.savemode = append_flag ? 'M' : FILE_TAGED;
...
delete_record(char *fpath, int size, int id)
{
  ...
  while (read(fdr, abuf, size) == size)
  {
    ...
    if (id != count++ && (safewrite(fdw, abuf, size) == -1))
    {
      ...
      bbslog("delete_record: %s failed!!! (safewrite)\n", fpath);
#ifdef  HAVE_REPORT
