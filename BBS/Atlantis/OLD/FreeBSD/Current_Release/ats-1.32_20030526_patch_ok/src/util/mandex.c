/*-------------------------------------------------------*/
/* util/mandex.c              ( MapleBBS SobVersion )    */
/*-------------------------------------------------------*/
...
char refuse_list[] = BBSHOME "/etc/refuse.checkman";
...
/*          1 ¤@¯ë¬ÝªO */
int
board_allow(char* bname)
{
  int i;
  for(i = 0; i < allb ; i++)
  {
    if (!strcmp(bname, brd[i].bname) && brd[i].level == 0)
      return 1;
    else if (!strcmp(bname, brd[i].bname))
      return 0;
  }
  return -1;
...
void mandex(level, num_header, fpath)
  int level;
...
void
b_record(char *fname, int space, int delay, int file, int dir)
{
  char buf[200];
  FILE *fp;
...
main(int argc, char *argv[]) {
  ...
  while (read(inf, &tmpbrd, sizeof(boardheader)) == sizeof(boardheader))
  {
    if (tmpbrd.brdname[0])
    {
       ...
       if (belong(refuse_list,tmpbrd.brdname) ||
           strstr(tmpbrd.brdname,"Material"))
         brd[allb].level = 16384;
       else
  ...
  if (fp = fopen(delay_list, "w"))
  {
    ...
    for(j = 0; j < count; j++)
    {
      ...
      else
      {
        if ( bdata[j].delay >= 30 )
          color = 33;
        else if ( bdata[j].delay >= 15 )
          color = 32;
        else
          color = 37;

          fprintf(fp, "          [1;%dm%-14s     %-4d %-3s[m\n", color, bdata[j].bname, bdata[j].delay, "¤Ñ");
      }
    }
    fclose(fp);
  }
