...
//  sethomedir(genbuf, u->userid);
  ...
  outs(
"[1;33m¡´¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w[42m   ¨Ï¥ÎªÌ¸ê®Æ   [40m¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¡´\n");
  prints("\
...
//,u->pagernum);
  ...
  hour = (diff/60)%24;
  min = diff%60;
  prints("  [32m¤W¯¸®É¶¡¡G[37m%d¤Ñ %d¤p®É %d¤À %d¬í  ¤À¨­(%d)\n",day,hour,min,secd,count_multi());
...
u_habit()
{
  ...
  move(2, 0);
  outs("½Ð¨Ì·Ó±zªº¨Ï¥Î²ßºD¦Û¦æ½Õ¾ã  (¡´¬°¶})¡G\n");
  move(4, 0);
...
//         habitstrings[i+12],((fbits >> i+12) & 1 ? "¡´" : "  "));
  ...
  clrtobot();
  while (getdata(b_lines - 1, 0, "½Ð«ö [A-G] ¤Á´«³]©w¡A«ö [Return] µ²§ô¡G",
      choice, 2, LCECHO, 0))
  {
    i = choice[0] - 'a';
    if (i < 0 || i >= NUMHABITS)
...
showplans(char* uid)
{
  ...
  getdata(22,0,"¬Ý [1]¦W¤ùÀÉ [2]Ã±¦WÀÉ [3]ºëµØ¤å³¹ [4]­Ó¤H§@«~? ",ans,4,LCECHO,0);
...
u_editfile()
{
  ...
  getdata(b_lines,0," ­n¬Ý­þ­ÓÀÉ®×¡H ",ans,4,LCECHO,0);
  if(ans[0] == 'I' || ans[0] == 'i')
  {
    clear();
    sprintf(buf,BBSHOME "/WWW/user/%s.html",cuser.userid); 
    more(buf);
    getdata(1,0,"ºô­¶ index ½s¿è(E) §R°£(D) ¨S¨Æ[Q] ",ans,2,LCECHO,0);
    if(ans[0] == 'e' || ans[0] == 'E')
    {
  ...
  if (ans[0] >= '1' && ans[0] <= '9')
  {
    ...
    getdata(1,0,"¦W¤ùÀÉ ½s¿è(E) §R°£(D) ¨S¨Æ[Q] ",ans,2,LCECHO,0);
    if(ans[0] == 'e' || ans[0] == 'E')
    {
      setutmpmode(EDITSIG);
      vedit(buf,NA);
...
//    showplans(cuser.userid);
  ...
  if (ans[0] == 'C' || ans[0] == 'c')
  {
    getdata(b_lines-1,0,"ªZ¾¹¦WºÙ¡G",buf,10,DOECHO,currutmp->cursor);
...
/*    
    ...
    getdata(1,0,"ªZ¾¹ ½s¿è(E) §R°£(D) ¨S¨Æ[Q] ",ans,2,LCECHO,0);
    if(ans[0] == 'e' || ans[0] == 'E')
    {
      setutmpmode(EDITING);
      vedit(buf,NA);
...
u_justify()
{
  ...
  {
    pressanykey("±zªº¨­¥÷½T»{¤w¸g§¹¦¨¡A¤£»Ý­«±H¨­¥÷»{ÃÒ«H¥ó¡C");
    return XEASY;
  }
  else              
    mail_justify(cuser);
...
/*
  getdata(b_lines - 1, 0, "±z¬O§_­n¶ñ¼gµù¥U³æ¶Ü(Y/N)¡H[N] ", ans, 3, LCECHO,0);
  if (ans[0] != 'y')
    return RC_FULL;
*/
  move(2, 0);
  ...
  /* wildcat test µù¥U³qª¾ */
  sysop_bbcall("¨Ï¥ÎªÌµù¥U %d : %s",usernum,cuser.userid);
  cuser.rtimes++;
...
u_list()
{
  ...
  {
    getdata(b_lines - 1, 0, "Æ[¬Ý [1]¯S®íµ¥¯Å (2)¥þ³¡¡H", genbuf, 2, DOECHO,0);
    if (genbuf[0] == '1')
    {
    ...
    getdata(b_lines - 1, 0, "Åã¥Ü [1]¯u¹ê©m¦W (2)¼ÊºÙ¡H", genbuf, 2, DOECHO,0);
    if (genbuf[0] == '1')
      showrealname = 1;
  }
  else
