  ...
  char *bname;
{
  ...
  if (brd_bad_guy(buf, cuser.userid))
     return 0;
...
reload_filmcache()
{
   ...
   else
   {
     ...
     for(id = (film->max_history - 1) ; id>0 ; id--)
     {
        num = rand() % id;
        sprintf(buf, "%s", film->notes[num] );
        sprintf(film->notes[num], "%s", film->notes[id] );
        sprintf(film->notes[id], "%s", buf );
     }     
...
reload_fcache() // reload from cache
{
   if (fcache->busystate)
   {
     /*sleep(1);*/
