...
#define BBSUSER2 "bbs"
#define BBSHOME "/bbs"
#define BAD_HOST "/bbs/etc/bad_host"
#define BOARDNAME       "紅樓頂尖"			/* 中文站名 */
#define MYHOSTNAME      "exbbs.ilantech.edu.tw"		/* 網路位址 */
#define MYIP		"140.111.79.2"
#define MAXACTIVE	(384)				/* 最大上線人數 */

#include <pwd.h>

char bbsprog[128], *bbsport;
...
{
  char buf[256];
#if (!defined(BSD44) && !defined(LINUX)) || (defined(LINUX) && (__GLIBC__ != 2))
...
#ifdef  HAVE_CHKLOAD
    sprintf(ptyibuf + 1, BANNER, BOARDNAME,MYHOSTNAME,maple, line + 5);
#else
    sprintf(ptyibuf + 1, BANNER, BOARDNAME,MYHOSTNAME,MYIP);
#endif
  ...
  register char *line;
{
  ...
  if ((fd = open(_PATH_UTMP, O_RDWR, 0)) >= 0)
  {
    ...
    (void) lseek(fd, (off_t)((long)(utmp_slot)), L_SET);
    ...
    if (ut.ut_name[0] && !strncmp(ut.ut_line, line, sizeof(ut.ut_line)))
    {
      ...
      (void) lseek(fd, -(off_t)((long)sizeof(struct utmp)), L_INCR);
    ...
    do
    {
      if (!*pid)
      {
        ...
        line[sizeof("/dev/ptyp") - 2] =
          "pqrsPQRSlmnoLMNOhijkHIJKcefgCEFG"[i & 31];
//          "pqrsPQRS"[i & 7];
        line[sizeof("/dev/ptyp") - 1] = "0123456789abcdefghijklmnopqrstuv"[(i >> 4)& 31];
#else
        line[sizeof("/dev/ptyp") - 2] =
          "klmnopqrstuvwxyzKLMNOPQRSTUVWXYZ"[(i >> 5) & 31];
...
//      if (strcmp(crypt(p, salt), pwd->pw_passwd)) {
  ...
  execl(bbsprog, "bbs", rhost, line, rname, bbsport, NULL);
...
#define KMEM    "/dev/kmem"
  ...
  if (lseek(kmem, (off_t)offset, L_SET) == -1)
  {
    close(kmem);
    return (1);
  }
  ...
  char *argv[];
{
  ...
  {
    static int ports[] = {3000, 3001, 3002, 3003, 3004, 3005};
