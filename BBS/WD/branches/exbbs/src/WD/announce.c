...
//  fileheader backup;
  ...
  if (pm->header[pm->now - pm->page].filename[0] == 'H' &&
	...
	return;
/*
      memcpy(&backup, &(pm->header[pm->now - pm->page]), sizeof(fileheader));
      setbfile (buf, (HAS_PERM(PERM_SYSOP)) ? "junk" : "deleted", backup.filename);
      f_mv(fpath, buf);
      setbdir (buf, (HAS_PERM(PERM_SYSOP)) ? "junk" : "deleted" );
     ...
     AMENU *pm;
{
  ...
  if (copyfile[0])
  {
    ...
    sprintf (buf, "½T©w­n«þ¨©[%s]¶Ü(Y/N)¡H[Y] ", copytitle);
...
a_backup (pm)
  AMENU *pm;
{
   char fname[128], buf[128], ans[3];
  time_t i;
  sprintf(buf, "%s", pm->path);
  buf[strlen(buf)]='\0';
  for(i=0 ; i<strlen(buf) ; i++)
     if(buf[i]=='/') { buf[i]='_'; }

  sprintf(fname, "backup/%s.tgz\0", buf);
  i=dasht(fname);
  if(i>=0)
  {
     sprintf(buf, "³Æ¥÷ÀÉ®×¤w¸g¦s¦b¡A³Æ¥÷®É¶¡¬O¡G%s", ctime(&i));
     outmsgline(buf, b_lines-2);
  }
  sprintf(buf, "¡i¿ï¶µ¡j(b)³Æ¥÷  ");
  if(i>=0)
    strcat(buf, "(d)§R°£  (r)ÁÙ­ì  ");
  strcat(buf,"? ");
   getdata (b_lines - 1, 0, buf, ans, 2, DOECHO, 0);  
   if(ans[0]=='b')
   {
     sprintf(buf, "tar zcf %s %s", fname, pm->path);
     outmsgline("backup and zip ...", b_lines-1);
     system(buf);
     if(dasht(fname) >= 0)
       pressanykey("¦¨¥\\!");
     else
   ...
   {
     f_rm(fname);
     outmsgline("delete file ...", b_lines-1);
   }
   else if(ans[0]=='r')
   {
     sprintf(buf, "tar zxf %s", fname);
     outmsgline("restore ...", b_lines-1);
     system(buf);
   }
   return;
}           
...
a_restore (pm)
  AMENU *pm;
{
//   char fname[MAXPATHLEN], buf[ANSILINELEN], ans[3];
//   int i;
  return;
}     
     ...
     AMENU *pm;
{
  ...
  switch (mode)
    ...
    case ADDGOPHER:
      ...
      if (!getdata (b_lines - 2, 1, "¿é¤JURL¦ì§}¡G", item.filename + 2, 61, DOECHO))
	return;
      strcpy (item.title, "¡¹ ");	/* A1BB */
      break;
    case ADDLINK:
...
//		  strcpy (item.title, fhdr->title + ((currutmp->pager > 1) ? 3 : 0));
	...
	case 'R':
	  if (me.now < me.num)
	    {
	      fileheader *fhdr = &me.header[me.now - me.page];
	      if ((fhdr->filemode & FILE_REFUSE) && (me.level < MANAGER))
		{
		  pressanykey ("³o¬O¨p¤H­«¦a  ¹ï¤£°_  §AµLªk¶i¥h");
		  me.page = 9999;
		  break;
		}
	      sprintf (fname, "%s/%s", path, fhdr->filename);
