...
multi_user_check()
{
  ...
  if (cuser.userlevel)
  {
    ...
    getdata(b_lines, 0, "±z·Q§R°£¨ä¥L­«½Æªº login ¶Ü¡H[Y] ", genbuf, 2, LCECHO,0);
...
// -END-
  ...
  while (1)
  {
    ...
    getdata(22, 19, "[±zªº±b¸¹] ",uid, IDLEN + 1 , DOECHO,0);
...
*/
    ...
    {
      getdata(22, 45, "[±zªº±K½X] ", passbuf, PASSLEN, PASS,0);
//      passbuf[PASSLEN] = '\0';
...
//          cuser.userlevel = ~0;
        ...
        {
           logattempt(cuser.userid, '*');
           pressanykey("[¨t²ÎÄµ§Ù] ¯S®í±b¸¹ ½Ð¥Ñ Trusted Port ¶i¤J ");
           continue;
...
/* ªO¥D´£°ª«H½c¤W­­ */
  ...
  {
    char *nick[10] = {"¸ô¹Lªº¯«","¸ô¹Lªº¿ß","¸ô¹Lªºª¯","¸ô¹Lªº½Þ","¸ô¹Lªº³¾",
                  "¸ô¹Lªº¹q¤lÂû","¦ó¶·°Ý§Ú¦W","¯«¯µ­I¼v","¹C»î","¥¢¾Ðªº³X«È"
                     };
    char *name[10] = {"¦ãºû§J", "µá¥§µ·", "³Íº¸", "¼¯¨½Äõ", "¦ãªÜµY",
                      "¾|©Z¹F", "®õ©Ô¨u", "¥ì¤Z", "¹F¥d"  , "Á¢®R"
                     };
    char *addr[10] = {"­·¨t", "¤g¨t", "¤õ¨t", "¹p¨t", "¤ô¨t",
                      "­·¨t", "¤g¨t", "¤õ¨t", "¹p¨t", "¤Ñ¨t"
                     };
...
waste_money()
{
    ...
    && cuser.numlogins > 2)
  {
    ...
    outs("½Ð·Q¿ìªkªá±¼¤@¨Ç , ©Î¬O§â¿úÂà¦¨ª÷¹ô§a!\n¦b°Ó«~¤jµóªº¹q¤l»È¦æ¤¤¦³´«¿úªº¿ï¶µ .");    
    pressanykey("§A»È¹ô¤Ó¦hÅo¡I·Q¿ìªkªá±¼§a¡I");
    finance();
    game_list();
  }
}
