...
*/
     ...
     namecomplete("½Ð¿é¤J¬ÝªO¦WºÙ¡G", genbuf);
     for (num = 0; num < brdnum; num++)
...
brdlist_foot()
{
  prints(COLOR2"  ¿ï¾Ü¬ÝªO  "COLOR1"[1m  [33m(v/V)[37m¼Ð°O¤wÅª/¥¼Åª  "
    "[33m(y)[33m¦C¥X[37m%s  [33m(z)[37m¨ú®ø/­q¾\\     [44m %13s [m",
    yank_flag ? "­q¾\\" : "¥þ³¡", cuser.userid);
}
/*
woju
*/
...
show_brdlist(head, clsflag, newflag)
{
  ...
  if (clsflag)
  {
    ...
    prints("[¡ö]¥D¿ï³æ [¡÷]¾\\Åª [¡ô¡õ]¿ï¾Ü [y]¸ü¤J [S]±Æ§Ç [/]·j´M [TAB]¤åºK¡E¬ÝªO [^Z]¨D§U\n"
      COLOR1"[1m%-22s Ãþ§O Âà %-32s§ë ªO    ¥D     [m",
...
//          if((ptr->brdattr & BRD_HIDE || !(ptr->brdattr & BRD_POSTMASK))
             ...
             (!HAS_PERM(PERM_SYSOP) && !is_BM(ptr->BM) ) )
          {
            ...
            {
//              pressanykey(P_BOARD);
              pressanykey("¹ï¤£°_¡A¦¹ªO¥u¤¹³\\ ¬ÝªO¦n¤Í ¶i¤J®@!");
              break;
            }
          }
