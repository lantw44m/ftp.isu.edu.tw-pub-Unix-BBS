  ...
  FILE *fp;
{
  ...
  if (!strcmp(cuser.userid, "guest") || bad_user(cuser.userid)) {
     fprintf(fp, "\n--\n[1;36m¡°Post by [37m%s [36mfrom [33m%s[0m",cuser.userid, getenv("RFC931"));
     return;
  }
...
//    fprintf(fp, "\n--\n[1;36m ²q²q§Ú¬O½Ö¡I\n");
//  else {
    strncpy (temp,fromhost,41);
    temp[42]='\0';
    fprintf(fp, "\n--\n[1;36m¡°Post by [37m%s [36mfrom [33m%s [0m\n",cuser.userid,temp);
//  }
//#else
  ...
  int saveheader;
{
  ...
  if (currstat == SMAIL)
    msg = "[S]Àx¦s (A)©ñ±ó (E)Ä~Äò (R/W/D)Åª¼g§R¼È¦sÀÉ¡H";
  else
    msg = "[S]Àx¦s (L)¯¸¤º (A)©ñ±ó (E)Ä~Äò (R/W/D)Åª¼g§R¼È¦sÀÉ¡H";
...
/* Jaky */
    if (count >= 200)
    {
       sprintf (buf,"[1;33;46m%s[37m¦b[37;45m%s[37mªO¯A¶û¹HªkÁÈ¿ú , %s[m",
       cuser.userid,currboard,ctime(&now));
       f_cat ("etc/illegal_money",buf);
//       degold(100);
//       abort_bbs();          
           /* ³sÄò¨â¦Ê¬í¤£ÃP¤â,¤À©ú¬O¦bÀÄ°]*/
    }
