...
extern int add_group();

static int mail_del(), mail_read(), mail_save();
static int m_forward(), multi_reply(), mail_cross_post();
...
// wildcat : bbs ©Ò¦³ domain name list
  char myhost[5][64] = {"exbbs.twbbs.org","exbbs.ilantech.com.tw","","","" }; 
  FILE *fp;
  int i=0,allow=1;
...
do_hold_mail(fpath, receiver, holder)
  ...
  char *holder;
{
  char title[128], buf[128];
  fileheader mymail;

  sprintf(buf, "home/%s/mailbox", holder);
  stampfile(buf, &mymail);
  mymail.savemode = mymail.filemode = 0;
  mymail.filemode |= MAIL_ALREADY_READ;
  strcpy(mymail.owner, "[³Æ¥÷«H¥ó]");
  if (receiver)
  {
  ...
  sprintf(title, "home/%s/mailbox/.DIR", holder);
  ...
  char *userid, *title;
{
  ...
  else
  {
   ...
   strcpy(userid, receiver);
    ...
    sprintf(genbuf, "home/%s/mailbox", userid);
    stampfile(genbuf, &mhdr);
    f_mv(fpath, genbuf);
    strcpy(mhdr.owner, cuser.userid);
    strncpy(mhdr.title, save_title, TTLEN);

    sprintf(fpath, "home/%s/mailbox/.DIR", userid);
...
/*
  ...
  if (uident[0])
    my_send(uident);
  log_usies("MAIL",uident);
  return 0;
*/
  char uident[40],buf[200];
  stand_title("­·±a¨Óªº®ø®§");
  usercomplete(msg_uid, uident);
  ...
  else if (isprint(uident[0]))
  {
     sprintf(buf,"home/%s/RejectMail", uident);
     if(belong(buf,cuser.userid))
       pressanykey("¨Ï¥ÎªÌ %s ©Ú¦¬«H¥ó",uident);
     else if (uident[0])
     {
       my_send(uident);
       log_usies("MAIL",uident);
...
/*³]©wÀÉ«H¦W³æ¡AÀH«K§ä¦U¦a¤è®a¶i¥h*/
RejectMail(char* uid)
{
    char ans[4],buf[200];
    setutmpmode(REJECTMAIL);
    showtitle("½s¾×«H¦W³æ", BoardName);
    setuserfile(buf,fn_RejectMail);
...
//        sethomepath(genbuf, p->word);
         sprintf(genbuf, "home/%s/mailbox", p->word);
      else
        continue;
      ...
      mymail.savemode |= FILE_MULTI_SEND; 
                                      /* multi-send flag */
//      sethomedir(genbuf, p->word);
       sprintf(genbuf, "home/%s/mailbox/.DIR", p->word);
...
//         sethomedir(genbuf, userid);
          sprintf(genbuf, "home/%s/mailbox/.DIR", userid);
         if (rec_add(genbuf, &mymail, sizeof(mymail)) == -1)
            outs(err_uid);
  ...
  fileheader *fptr;
{
  ...
  fptr->filemode |= MAIL_ALREADY_READ;
...
mailtitle()
{
  ...
  outs("[¡ö]Â÷¶} [¡ô,¡õ]¿ï¾Ü [¡÷]¾\\Åª [z]«H¥óºëµØ°Ï [^X] Âà¿ý¦Ü¬ÝªO [^Z]¨D§U\n");
  outs("[1m"COLOR1"  ½s¸¹   ¤é ´Á  §@ ªÌ          «H  ¥ó  ¼Ð  ÃD ");
  ...
  sprintf(buf,"%s%*s[m",buf,33-strlen(buf),"");
  outs(buf);
}
  ...
  char *direct;
{
 ...
 if(strncmp(fhdr->filename,"D",1)==0)
 {
   strcat(buf, "/.DIR");
//   i_read(RMAIL, buf, currtitle, doent, mail_comms, NULL);
   return RC_FULL;
 }
 else
 {
   ...
   while (!done)
   {
    ...
    if (more_result != -1) {
       fhdr->filemode |= MAIL_ALREADY_READ;
...
//  char genbuf[256];
  ...
  if (HAS_PERM(PERM_SYSOP) || !strcmp(fhdr->owner, cuser.userid))
  {
    ...
    {
      ent = 0;
      getdata(2, 0, "«O¯d­ì§@ªÌ¦WºÙ¶Ü¡H[Y] ", ans, 2, DOECHO,"Y");
      if (ans[0] != 'n' && ans[0] != 'N') author = 1;
    }
  }
...
//  }
  ...
  getdata(2, 0, "(S)¦sÀÉ (L)¯¸¤º (Q)¨ú®ø¡H[S] ", ans, 2, LCECHO,"S");
  if (ans[0] == 'l' || ans[0] == 's')
  {
...
//    setuserfile(fname, fhdr->filename);
    sprintf(fname , "home/%s/mailbox/%s", cuser.userid, fhdr->filename);
    if (ent)
    {
      xptr = fopen(xfpath, "w");
...
//    sethomepath(genbuf, hacker);
    sprintf(genbuf, "home/%s/mailbox", hacker);
    stampfile(genbuf, &mymail);
...
//    sethomedir(genbuf, hacker);
    sprintf(genbuf, "home/%s/mailbox/.DIR", hacker);
  ...
  int mode;                     /* ¬O§_ uuencode */
{
  ...
  if (mode == 'Z') {
     ...
     sprintf(fname, "cd home; tar zcf - %s | uuencode %s.tgz > /tmp/%s.tgz",
