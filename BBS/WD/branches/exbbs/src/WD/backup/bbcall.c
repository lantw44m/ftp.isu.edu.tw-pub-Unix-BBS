...
alpha0943()
{
  ...
  if ((!getdata(0, 0, "請輸入您要傳呼的號碼：0943-", ID, 7, DOECHO,0) &&
       !getdata(1, 0, "請輸入您要傳呼的代號：", Name, 32, DOECHO,0)) ||
         !getdata(2, 0, "請輸入傳呼訊息：", Msg, 64, DOECHO,0))
  {
    pressanykey("放棄傳呼");
    return;
  }
...
alpha0946()
{
  ...
  if ((!getdata(0, 0, "請輸入您要傳呼的號碼：0946-", ID, 7, DOECHO,0) &&
       !getdata(1, 0, "請輸入您要傳呼的代號：", Name, 32, DOECHO,0)) ||
         !getdata(2, 0, "請輸入傳呼訊息：", Msg, 64, DOECHO,0))
  {
    pressanykey("放棄傳呼");
...
fitel()
{
  ...
  if (!getdata(0, 0, "請輸入您要傳呼的號碼：0948-", svc_no, 7, DOECHO,0) ||
       !getdata(1, 0, "請輸入傳呼訊息：", message, 61, DOECHO,0))
  {
    pressanykey("放棄傳呼");
    return;
  }
  ...
  sprintf(trn, "svc_no=%s&pwd_in=%s&isp_code=%s&sender_name=%s&message=%s&reminder_option=%s&year=88&month=01&day=01&hour=00&min=00\n",
