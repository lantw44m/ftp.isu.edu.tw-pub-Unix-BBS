...
// wildcat : 最多可以 multi-post 多少個 group ?
#define MAX_GROUP	3
...
receive_article()
{
  ...
  int          toklen, i;
  char         tmpbuf[60];
// wildcat : M3 like -> for anti- multi-post
  char *data = GROUPS;
  int cc = 0,rel = 0;

  now = time(0);

// wildcat : anti- multi-post ...
#if 1
  for (;;)
  {
    cc = *data++;
    ...
    if (cc == ',')
    {
      if (++rel > MAX_GROUP)	/* 超過 ? 個 news groups */
      {
        bbslog("SPAM : Too Much Group , <%s> \n",MSGID);
        return 0;
      }
    }
  }
