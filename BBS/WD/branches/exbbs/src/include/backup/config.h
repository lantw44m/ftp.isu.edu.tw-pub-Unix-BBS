/*-------------------------------------------------------*/
/* config.h     ( WD-BBS Ver 2.3 )			 */
/*-------------------------------------------------------*/
/* target : site-configurable settings                   */
/* create : 95/03/29                                     */
/* update : 98/12/09                                     */
/*-------------------------------------------------------*/

#ifndef _CONFIG_H_
#define _CONFIG_H_

/* ----------------------------------------------------- */
/* ©w¸q BBS ¯¸¦W¦ì§}                                     */
/* ------------------------------------------------------*/

#define BOARDNAME       "¬õ¼Ó³»¦y"			/* ¤¤¤å¯¸¦W */
#define BBSNAME         "NIIT EXBBS"			/* ­^¤å¯¸¦W */
#define MYHOSTNAME      "exbbs.ilantech.edu.tw"		/* ºô§} */
#define MYIP		"140.111.79.2"			/* IP */
#define MYVERSION       "WD-BBS 2.3"			/* µ{¦¡ª©¥» */
#define MYBBSID         "EXBBS"				/* Âà«H¥N½X */
#define BBSHOME         "/bbs"				/* BBS ªº®a */
#define BBSUSER         "bbs"
#define BBSUID          999
#define BBSGID          900
#define BBSSHELL        "/bbs/bin/bbsrf"		/* «eºÝµ{¦¡ */
#define BBSPROG         "/bbs/bin/bbs"			/* ¥Dµ{¦¡ */
#define TAG_VALID	"[EX BBS]To "

#define CURSOR_BAR
#ifdef CURSOR_BAR
 #define BAR_COLOR "[1;33;46m"
#endif


/* ----------------------------------------------------- */
/* ²ÕºA³W¹º                                              */
/* ------------------------------------------------------*/

#define HAVE_CHKLOAD		/* check cpu load */
#ifdef HAVE_CHKLOAD
  #define MAX_CPULOAD     (32)            /* CPU ³Ì°ªload */
  #define MAX_SWAPUSED    (32)            /* SWAP³Ì°ª¨Ï¥Î²v */
#endif

#define WITHOUT_CHROOT		/* ¤£»Ý­n root set-uid */
#define HAVE_MOVIE		/* Åã¥Ü°ÊºA§i¥ÜªO */
#define INTERNET_PRIVATE_EMAIL	/* ¥i¥H±H¨p¤H«H¥ó¨ì Internet */
#define BIT8			/* ¤ä´©¤¤¤å¨t²Î */
#define DEFAULTBOARD	"SYSOP"	/* ¹w³]¬ÝªO */
#define LOGINATTEMPTS	(3)	/* ³Ì¤j¶i¯¸¥¢»~¦¸¼Æ */
#define LOGINASNEW		/* ±Ä¥Î¤W¯¸¥Ó½Ð±b¸¹¨î«× */
#define VOTE		(1)	/* ´£¨Ñ§ë²¼¥\¯à */
#define INTERNET_EMAIL		/* ¤ä´© InterNet Email ¥\¯à(§t Forward) */
#define EMAIL_JUSTIFY		/* µo¥X InterNet Email ¨­¥÷»{ÃÒ«H¨ç */
#undef  NEWUSER_LIMIT		/* ·s¤â¤W¸ôªº¤T¤Ñ­­¨î */
#define POSTNOTIFY		/* ·s¤å³¹³qª¾¥\¯à */
#undef  INVISIBLE		/* ÁôÂÃ BBS user ¨Ó¦Û¦ó³B */
#define MULTI_NUMS	(2)	/* ³Ì¦h­«½Æ¤W¯¸¤H¦¸ (guest°£¥~) */
#define MULTI_GUEST	(16)	/* ³Ì¦h­«½Æ¤W¯¸ªºguest */
#define INITIAL_SETUP		/* ­è¶}¯¸¡AÁÙ¨S«Ø¥ß¹w³]¬ÝªO[SYSOP] */
#define HAVE_MMAP		/* ±Ä¥Î mmap(): memory mapped I/O */
#define HAVE_ANONYMOUS		/* ´£¨Ñ Anonymous ªO */
#define HAVE_ORIGIN		/* Åã¥Ü author ¨Ó¦Û¦ó³B */
#define HAVE_MAILCLEAN		/* ²M²z©Ò¦³¨Ï¥ÎªÌ­Ó¤H«H½c */
#define HAVE_SUICIDE		/* ´£¨Ñ¨Ï¥ÎªÌ¦Û±þ¥\¯à */
#define WHERE			/* ¬O§_¦³¬G¶m¥\¯à */
#define HAVE_NOTE_2		/* wildcat:¤p¬ÝªO¥\¯à */

/* ----------------------------------------------------- */
/* ÀH BBS ¯¸³W¼Ò¦¨ªø¦ÓÂX¼W                               */
/* ----------------------------------------------------- */

#define MAXUSERS        (65536)         /* ³Ì°ªµù¥U¤H¼Æ */
#define MAXBOARD        (512)           /* ³Ì¤j¶}ª©­Ó¼Æ */
#define MAXACTIVE       (512)           /* ³Ì¦h¦P®É¤W¯¸¤H¼Æ */
#define MAX_FRIEND      (128)           /* ¸ü¤J cache ¤§³Ì¦hªB¤Í¼Æ¥Ø */
#define MAX_REJECT      (128)            /* ¸ü¤J cache ¤§³Ì¦hÃa¤H¼Æ¥Ø */
#define MAX_LOGIN_INFO  (128)           /* ³Ì¦h¤W½u³qª¾¤H¼Æ */
#define MAX_POST_INFO   (128)            /* ³Ì¦h·s¤å³¹³qª¾¤H¼Æ */
#define MAX_NAMELIST    (128)           /* ³Ì¦h¨ä¥L¯S§O¦W³æ¤H¼Æ */
#define MAX_MOVIE       (128)		/* ³Ì¦h°ÊºA¬Ýª©¼Æ */
#define MAX_FROM        (128)		/* ³Ì¦h¬G¶m¼Æ */
#define MAX_REVIEW      (32)            /* ³Ì¦h¤ô²y¦^ÅU */

/* ----------------------------------------------------- */
/* ¨ä¥L¨t²Î¤W­­°Ñ¼Æ                                      */
/* ----------------------------------------------------- */

#define MAX_HISTORY     64		/* °ÊºA¬ÝªO«O«ù 12 µ§¾ú¥v°O¿ý */
#define MAXKEEPMAIL     (100)		/* ³Ì¦h«O¯d´X«Ê MAIL¡H */
#define MAXEXKEEPMAIL   (500)		/* ³Ì¦h«H½c¥[¤j¦h¤Ö«Ê */
#define MAX_NOTE        (64)            /* ³Ì¦h«O¯d´X½g¯d¨¥¡H */
#define MAXSIGLINES     (8)             /* Ã±¦WÀÉ¤Þ¤J³Ì¤j¦æ¼Æ */
#define MAXQUERYLINES   (16)            /* Åã¥Ü Query/Plan °T®§³Ì¤j¦æ¼Æ */
#define MAXPAGES        (900)           /* more.c ¤¤¤å³¹­¶¼Æ¤W­­ (lines/22) */
#define MAXMSGS         (16)		/* ¤ô²y(¼ö°T)§Ô­@¤W­­ */
#define MOVIE_INT       (12)		/* ¤Á´«°Êµeªº¶g´Á®É¶¡ 12 ¬í */

/* ----------------------------------------------------- */
/* µo§b¹L¤[¦Û°ÊÃ±°h    */
/* ------------------------------------------------------*/

#define DOTIMEOUT

#ifdef  DOTIMEOUT
#define IDLE_TIMEOUT    (29*60)		/* ¤@¯ë±¡ªp¤§ timeout */
// #define MONITOR_TIMEOUT (29*60)         /* monitor ®É¤§ timeout */
#define SHOW_IDLE_TIME                  /* Åã¥Ü¶¢¸m®É¶¡ */
#endif

/* ----------------------------------------------------- */
/* chat.c & xchatd.c ¤¤±Ä¥Îªº port ¤Î protocol           */
/* ------------------------------------------------------*/

#define CHATPORT	3838

#define MAXROOM         32             /* ³Ì¦h¦³´X¶¡¥]´[¡H */

#define EXIT_LOGOUT     0
#define EXIT_LOSTCONN   -1
#define EXIT_CLIERROR   -2
#define EXIT_TIMEDOUT   -3
#define EXIT_KICK       -4

#define CHAT_LOGIN_OK       "OK"
#define CHAT_LOGIN_EXISTS   "EX"
#define CHAT_LOGIN_INVALID  "IN"
#define CHAT_LOGIN_BOGUS    "BG"

#define BADCIDCHARS " *:/\,;.?"        /* Chat Room ¤¤¸T¥Î©ó nick ªº¦r¤¸ */


/* ----------------------------------------------------- */
/* ¨t²Î°Ñ¼Æ      cache                                   */
/* ----------------------------------------------------- */
#define MAGIC_KEY       1999   /* ¨­¤À»{ÃÒ«H¨ç½s½X */

#define SEM_ENTER      -1      /* enter semaphore */
#define SEM_LEAVE      1       /* leave semaphore */
#define SEM_RESET      0       /* reset semaphore */

#define BRDSHM_KEY      1215
#define UIDSHM_KEY      1217
#define UTMPSHM_KEY     1219
#define FILMSHM_KEY     1221    /* °ÊºA¬Ýª© , ¸`¤é */
#define FROMSHM_KEY     1223    /* whereis, ³Ì¦h¨Ï¥ÎªÌ */

#define BRDSEM_KEY      2005
#define FILMSEM_KEY     2000    /* semaphore key */
#define FROMSEM_KEY     2003    /* semaphore key */

/* ----------------------------------------------------- */
/* ¥Ó½Ð±b¸¹®É­n¨D¥Ó½ÐªÌ¯u¹ê¸ê®Æ                          */
/* ----------------------------------------------------- */

#define SHOWUID                 /* ¬Ý¨£¨Ï¥ÎªÌ UID */
#define SHOWTTY                 /* ¬Ý¨£¨Ï¥ÎªÌ TTY */
#define SHOWPID                 /* ¬Ý¨£¨Ï¥ÎªÌ PID */

#define REALINFO                /* ¯u¹ê©m¦W */

#ifdef  REALINFO
#undef  ACTS_REALNAMES          /* ¥D¥Ø¿ýªº (U)ser Åã¥Ü¯u¹ê©m¦W */
#undef  POST_REALNAMES          /* ¶K¤å¥ó®Éªþ¤W¯u¹ê©m¦W */
#undef  MAIL_REALNAMES          /* ±H¯¸¤º«H¥ó®Éªþ¤W¯u¹ê©m¦W */
#undef  QUERY_REALNAMES         /* ³Q Query ªº User §iª¾¯u¹ê©m¦W */
#endif

/* ----------------------------------------------------- */
/* http                                                  */
/* ----------------------------------------------------- */

#define USE_LYNX   	      /* ¨Ï¥Î¥~³¡lynx dump ? */
#undef USE_PROXY
#ifdef  USE_PROXY
#define PROXYSERVER "140.111.72.8"
#define PROXYPORT   3128
#endif

#define LOCAL_PROXY           /* ¬O§_¶}±Òlocal ªºproxy */
#ifdef  LOCAL_PROXY
#define HPROXYDAY   3         /* localªºproxy refresh¤Ñ¼Æ */
#endif

/* ----------------------------------------------------- */
/* ´£¨Ñ¥~±¾µ{¦¡                                          */
/* ----------------------------------------------------- */

#define HAVE_EXTERNAL

  #ifdef HAVE_EXTERNAL
    #undef HAVE_TIN        /* ´£¨Ñ news reader */
    #undef HAVE_GOPHER     /* ´£¨Ñ gopher */
    #undef HAVE_WWW        /* ´£¨Ñ www browser */
    #define HAVE_BIG2       /* ´£¨Ñ big2 game */
    #define HAVE_MJ         /* ´£¨Ñ mj game */
    #define HAVE_BRIDGE     /* ´£¨Ñ Ok Bridge */
    #undef HAVE_MUD        /* ´£¨Ñ MUD */
    #define HAVE_CAL        /* ´£¥\­pºâ¾÷ */
    #undef HAVE_ARCHIE     /* have arche */
    #define BBSDOORS        /* ´£¨Ñ bbs doors */
    #define HAVE_GAME       /* ´£¨Ñºô¸ô³s½u¹CÀ¸ */
    #define HAVE_ADM_SHELL  /* ´£¨Ñ csh ¥H«KºÞ²z */
  #endif

#endif

/* ----------------------------------------------------- */
/* ©w¸q°ò¥»ÃC¦â                                          */
/* ----------------------------------------------------- */

#define ANNOUNCE_BRD	"Announce"
#define VOTEBOARD	"VoteBoard"

#define COLOR1	"[46;37m"
#define COLOR2	"[1m[44;33m"
