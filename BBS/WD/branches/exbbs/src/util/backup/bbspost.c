...
//  struct stat st;
  ...
  rec_get(BOARDS_REC, &bh, sizeof(boardheader), bnum);

  if( bh.brdattr & BRD_HIDE )
  {
    sprintf(buf,"Permission denied(hide) - [%s]",bh.brdname);
    postlog(buf);
    return -1;
  }
  if(!(bh.brdattr & BRD_BRDMAIL))
  {
    sprintf(buf,"Permission denied(BRDMAIL) - [%s]",bh.brdname);
    postlog(buf);
    return -1;
  }
