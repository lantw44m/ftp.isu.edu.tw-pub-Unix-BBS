...
start:
  ...
  if(mode==1)
        strcpy(buf,"etc/top_group.list");
  else
        strcpy(buf,"etc/top_group.table");
 ...
 for(i=0;i<num;i++)
 {
   ...
   if(mode==1)
     fprintf(op,"[1m%4d[m %-15.15s%-28.28s %5d %8d %9d\n",
     i+1,st[i].boardname,st[i].expname,st[i].times,st[i].sum,
   ...
   else
   {
     ...
     i+1,st[numboards-1].boardname,st[numboards-1].expname,
       st[numboards-1].times,st[numboards-1].sum,
