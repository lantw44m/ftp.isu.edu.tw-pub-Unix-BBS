...
#define POP3_PORT       1111
#define POP3_HOME       (BBSHOME "/boards")
#define POP3_TIMEOUT    (60 * 30)

#define POP3_LOGFILE    BBSHOME "/log/brdpop3d.log"
#define POP3_PIDFILE    BBSHOME "/log/brdpop3d.pid"

#define POP3_FQDN       ".board@" MYHOSTNAME

#define POP3_FREEDOM    5

#define SOCK_QLEN       10
#define SOCK_TIMEOUT    (30 * 60)


#define SND_TIMEOUT     30
...
typedef struct
{
  char filename[FNLEN - 8];     /* M.9876543210.A */
  char *author;
  int mailsize;
...
typedef struct boardheader ACCT;
  ...
  Client *cn;
{
  ...
  if (userecno = getbnum(userid))
  {
    fseek(fp_passwd, --userecno * sizeof(acct), 0);
    fread(&acct, sizeof(acct), 1, fp_passwd);
    strcpy(cn->userid, acct.brdname);
    if(acct.level > PERM_LOGINOK || acct.brdattr & BRD_HIDE)
      memcpy(cn->passwd, acct.passwd, PASSLEN);
    else
      memcpy(cn->passwd, genpasswd("WD"), PASSLEN);
    sprintf(msg, "+OK Password required for %s%s", acct.brdname, POP3_FQDN);
  }
  sprintf(cn->home, "%s/", acct.brdname);
  ...
  Client *cn;
{
  ...
  if (n = do_number(cn, 0))
  {
    ...
    else
    {
      ...
      {
        str = data;
        sprintf(str, "%s%s", cn->userid, POP3_FQDN);
      }
...
{
  client_puts(cn, "+OK");
  client_puts(cn, "list         list mail");
  client_puts(cn, "stat         mail stat");
  client_puts(cn, "retr <num>   retrive mail");
  client_puts(cn, "dele <num>   delete mail");
  client_puts(cn, "uidl");
  client_puts(cn, "last");
  client_puts(cn, "user");
  client_puts(cn, "pass");
...
main()
{
  ...
  if (!(fp_passwd = fopen(fn_passwd, "r"))) {
     logit("EXIT", ".BOARDS opened error");
     syslog(LOG_NOTICE, ".BOARDS opened error, abort server\n");
     abort_server();
  }
