...
p_race()
{
  char bet[1],m1[8],ch,*racename[5]={"¿}¿}","£«½÷","±j­ô","³ÍÂÈ","Ãý©É"};
  ...
  currutmp->pager =2;
	...
	outs("\n  [1m³t«×¡G[m    \n  [1m½äª÷¡G[m    \n");
	outs("¡]¡@½Ð«ö [1;36mk[m ¬°§A¿ïªº¤p½Þ¥[ªo¡A«ö [1;36mz[m ¥i¥á¥X«OÄÖ²y(¥u¦³¤@¦¸¾÷·|)¡@¡^");
	move(9,0);
	for(i=0;i<5;i++)
	...
	{
	    do{
		getdata(0,0,"§A­n©ã­þ°¦(1~5)[S]¶}©l[Q]Â÷¶}¡G",bet,2,DOECHO,0);
		i=atoi(bet);
		if(i>0&&i<6)
		{
	...
	while(win==-1)
	{
	    ...
	    while(!((ch=='k'||ch=='K')||((ch=='z'||ch=='Z')&&ball==-1)));
            if(ch=='z'||ch=='Z')
            {
                ...
                prints("[1m«OÄÖ²y¯{¨ì[3%dm%s[37m°±¤î«e¶i¤T¦¸¡A³t«×=0[m",run+1,racename[run]);
		...
		if( now%12==0 && (flag<11||(flag<12&&ball==-2)) )
		{
		    ...
		    {
			case 0:
			    prints("ªA¤U«Â¦Ó­è¡A³t«×x1.5[m");
			    rate[run]*=1.5;
			    break;
			case 1:
			    prints("¨Ï¥XºµªºÃzµo¤O¡A«e¶i¤­®æ[m");
			...
			case 2:
			    ...
			    break;
			case 3:
			    prints("½ò¨ì­»¿¼¥Ö·Æ­Ë¡A¼È°±¤G¦¸[m");
			    stop[run]+=2;
			    break;			
			case 4:
			    prints("½Ð¯«¤W¨­¡A¼È°±¥|¦¸¡A³t«×¥[­¿[m");
			    stop[run]+=4;rate[run]*=2;
			    break;
			case 5:
			    if(p[0]+p[1]+p[2]+p[3]+p[4]>8000)
			    {
				prints("¥Ñ©ó SugarII §@¹ú¡Aª½¹F²×ÂI[m");
				win=race_path(run,p[run]/100,(3100-p[run])/100);
				p[run]=3001;
			    }
			    else
			    {
				prints("°Û¥X¤jÅ]ªk©G¡A¨Ï¨ä¥L¤H¼È°±¤T¦¸[m");
				for(i=0;i<5&&i!=run;i++)
				stop[i]+=3;
			    }
			    break;
			case 6:
			    prints("Å¥¨£ badboy ªº¥[ªoÁn¡A³t«×+100[m");
			    rate[run]+=100;
			    break;
			case 7:
			    prints("¨Ï¥X¿ûÅKÅÜ¨­¡A«e¶i¤T®æ¡A³t«×+30[m");
			    win=race_path(run,p[run]/100,3);
			    rate[run]+=30;
			    break;
			case 8:
			    prints("°I¯«¤W¨­³t«×´î¥b¡A®ÇÃä¼È°±¤G¦¸[m");
			    rate[run]/=2;
			    if(run>0) stop[run-1]+=2;
			    if(run<4) stop[run+1]+=2;
			    break;
			case 9:
			    prints("³Q SugarII ¶A©G¡A¦^¨ì°_ÂI[m");
			    win=race_path(run,p[run]/100,-30);
			    break;
			case 10:
			    if(p[0]+p[1]+p[2]+p[3]+p[4]>6000)
			    {
				prints("[5m¨Ï¥X³Í¤ÆÂÈ³³¡A©Ò¦³¤H¦^¨ì°_ÂI[m");
				for(i=0;i<5;i++)
				win=race_path(i,p[i]/100,-30);
			    }
			    else
			    {
				prints("¨Ï¥X¥øÃZ¼u¸õ¡A³t«×x1.3¡A¨ä¥L¤H´î¥b[m");
			...
			case 12:
			    while(run==now%5)
				now=random();
			    prints("·R¤W¤F[3%dm%s[36m¡A³t«×¸ò¨e¤@¼Ë",now%5+1,racename[now%5]);
			    rate[run]=rate[now%5];
			    break;
			case 13:
			    if(money[run+1]>0)
