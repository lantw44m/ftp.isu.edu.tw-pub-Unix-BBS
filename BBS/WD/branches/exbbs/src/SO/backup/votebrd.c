...
do_voteboardreply(fileheader *fhdr)
{
   ...
   do{
      clear();
      if (!getdata(18, 0, "請問您 (Y)支持 (N)反對 這個議題：", opnion, 3,LCECHO,0))
      {
         flock(fd, LOCK_UN);
         close(fd);
         fclose(fo);
   ...
   if (!getdata(20, 0, "請問您與這個議題的關係或連署理由為何：", reason,40, DOECHO,0))
    {
      flock(fd, LOCK_UN);
      close(fd);
      fclose(fo);
