...
bbcall_adduser (int max)
{
  ...
  if (genbuf[0] != ' ' && !belong (fpath, genbuf))
	...
	sprintf (newfriend.phone, "%s", "¥¼¿é¤J¹q¸Ü¸ê®Æ");
      y_pos += 2;
      if (getdata (y_pos, 0, "¡¯½Ð¿é¤J¤â¾÷¸¹½X¡]¨Ò 0943-1234556 ©Î 0943-¥N¸¹¡^¡G", newfriend.bbcall, 21, DOECHO, 0))
	{
	  y_pos++;
	  if (!getdata (y_pos, 0, "¡¯½Ð¿é¤J±K½X¡]¨S¦³ªº¸Ü§K¶ñ¡^¡G", newfriend.callpass, 11, DOECHO, 0))
	    {
...
bbcall_changeuser (struct BBCALL_DATA *newfriend)
{
  ...
  if (!getdata (y_pos, 0, "¡¯½Ð¿é¤J¹q¸Ü¡]¨Ò 03-5712121-12345¡^\n¡G", newfriend->phone, 19, DOECHO, ((strstr (newfriend->phone, "¥¼¿é¤J¹q¸Ü¸ê®Æ")) != NULL) ? 0 : newfriend->phone))
    sprintf (newfriend->phone, "%s", "¥¼¿é¤J¹q¸Ü¸ê®Æ");
  y_pos += 2;
  if (getdata (y_pos, 0, "¡¯½Ð¿é¤J¤â¾÷¸¹½X¡]¨Ò 0941-1234556 ©Î 0943-¥N¸¹¡^¡G", newfriend->bbcall, 21, DOECHO, ((strstr (newfriend->bbcall, "µL")) != NULL) ? 0 : newfriend->bbcall))
    {
      y_pos++;
      if (!getdata (y_pos, 0, "¡¯½Ð¿é¤J±K½X¡]¨S¦³ªº¸Ü§K¶ñ¡^¡G", newfriend->callpass, 11, DOECHO, ((strstr (newfriend->callpass, "µL")) != NULL) ? 0 : newfriend->callpass))
	{
	  strcpy (newfriend->callpass, "µL±K½X");
	}
    }
  ...
  do
      ...
      if (screen_refresh)
	{
	  ...
	  switch (screen_mode)
	    {
	    case 0:
	      prints ("[1;37;42m ½s¸¹  ¦W¦r©Î¥N¸¹  ¤À  Ãþ ¥Í ¤é ¹q¸Ü               ¤â¾÷¸¹½X     ±K½X      [0m");
	      break;
	    case 1:
	...
	case 'B':
	  if (data_count > 0)
	      ...
	      if ((strstr (bbcall, "µL¶Ç©I")) == NULL && bbcall[4] == '-' && (nummode == 941 || nummode == 943 || nummode == 945 || nummode == 946 || nummode == 947 || nummode == 948 || nummode == 949))
		{
	      ...
	      else if ((strstr (bbcall, "µL¤â¾÷")) != NULL)
		pressanykey ("¦¹¤H¨S¦³¤â¾÷¸¹½Xªº¸ê®Æ³á");
	      else
