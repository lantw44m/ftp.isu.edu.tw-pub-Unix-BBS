...
x_cdict()
{
    ...
    clear();
    counter(BBSHOME"/log/counter/cdict","¹q¤l¦r¨å",0);
    outs("\n[1mÅwªï¨Ï¥Î [32m­^º~¦r¨å[m ¥\\¯à\n");
...
x_tetris()
{
  ...
  int save_pager;
  clear();
  counter(BBSHOME"/log/counter/tetris","ª±«XÃ¹´µ¤è¶ô",0);
  pressanykey(NULL);
  save_pager = currutmp->pager;
  currutmp->pager = 2;
  setutmpmode(TETRIS);
...
x_mj()
{
  ...
  counter(BBSHOME"/log/counter/mj","¥´³Â±N",0);
  save_pager = currutmp->pager;
  currutmp->pager = 2;
  setutmpmode(MJ);
  demoney(100);
...
x_big2()
{
  ...
  int save_pager;
  if(check_money(100,SILVER)) return;
  counter(BBSHOME"/log/counter/big2","¥´¤j¦Ñ¤G",0);
  save_pager = currutmp->pager;
  currutmp->pager = 2;
  setutmpmode(BIG2);
  demoney(100);
...
x_chess()
{
  ...
  int save_pager;
  if(check_money(100,SILVER)) return;
  counter(BBSHOME"/log/counter/chess","¤U¶H´Ñ",0);
  save_pager = currutmp->pager;
  currutmp->pager = 2;
  setutmpmode(CHESS);
  demoney(100);
